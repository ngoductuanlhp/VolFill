...
class DINOv2Encoder(nn.Module):
    ...
    def forward(self, image: torch.Tensor, token_rows: Union[int, torch.LongTensor], token_cols: Union[int, torch.LongTensor], return_class_token: bool = False, return_register_tokens: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        features = self.backbone.get_intermediate_layers(
            image_14, n=self.intermediate_layers,
            return_class_token=True,
            return_register_tokens=return_register_tokens,
        )

        # Project patch features to the desired dimensionality
        if return_register_tokens:
            # features[i] = (patch_tokens, cls_token, reg_tokens)
            x = torch.stack([
                proj(feat.permute(0, 2, 1).unflatten(2, (token_rows, token_cols)).contiguous())
                    for proj, (feat, clstoken, regtokens) in zip(self.output_projections, features)
            ], dim=1).sum(dim=1)
            if return_class_token:
                return x, features[-1][1], features[-1][2]
            return x, features[-1][2]
        else:
            # features[i] = (patch_tokens, cls_token) — original behavior
            x = torch.stack([
                proj(feat.permute(0, 2, 1).unflatten(2, (token_rows, token_cols)).contiguous())
