from typing import *
from typing import IO  # not exported by ``from typing import *`` on 3.8.
...
class MoGeModel(nn.Module):
    ...
    def forward_semantics_and_geometry(
        self,
        image: torch.Tensor,
        ...
        """Single encoder pass at native patch resolution (img_h // 14).

        Runs the encoder once and fans out to both semantic tokens (for
        cross-attention conditioning) and fully post-processed geometry
        outputs (same format as infer()).

        Returns:
            semantics:  (B, N, encoder_dim) patch tokens for cross-attention.
            cls_token:  (B, encoder_dim).
        ...
        fp16_ctx = torch.autocast(device_type=device.type, dtype=torch.float16,
                                  enabled=use_fp16 and dtype != torch.float16)

        # ---- Encoder ----
        with fp16_ctx:
            enc_features, cls_token = self.encoder(image, base_h, base_w, return_class_token=True)

        # ---- Semantic branch (capture before UV concat) ----
        semantics = enc_features.clone().float().flatten(2).transpose(1, 2)  # (B, N, encoder_dim)

        # ---- Geometry branch: neck + heads ----
        if not (hasattr(self, 'points_head') and hasattr(self, 'mask_head')):
            return semantics, cls_token.float(), {}

        with fp16_ctx:
            feats = [enc_features, None, None, None, None]
            for level in range(5):
                ...
                feats[level] = torch.concat([feats[level], uv], dim=1) if feats[level] is not None else uv
            feats = self.neck(feats)
            points_raw = self.points_head(feats)[-1]
            mask_raw   = self.mask_head(feats)[-1]
            metric_scale = self.scale_head(cls_token).squeeze(1).exp() if hasattr(self, 'scale_head') else None

        # Resize to input resolution
        points_raw, mask_raw = (
            F.interpolate(v, (img_h, img_w), mode='bilinear', align_corners=False, antialias=False)
            for v in [points_raw, mask_raw]
        )

        # ---- fp32 post-processing (mirrors infer) ----
        with torch.autocast(device_type=device.type, dtype=torch.float32):
            points = points_raw.float().permute(0, 2, 3, 1)
            points = self._remap_points(points)
            mask_binary = mask_raw.float().squeeze(1).sigmoid() > 0.5
    ...
    def forward_semantics(self, image: torch.Tensor) -> torch.Tensor:
        """
        Extract patch-level semantic tokens from the frozen DINOv2 encoder.

        Uses a square patch grid aligned with the image's native patch size (stride 16).
        Returns (B, H//16 * W//16, encoder_dim) tokens for cross-attention conditioning.
        """
        batch_size, _, img_h, img_w = image.shape
        base_h, base_w = img_h // 14, img_w // 14
        features, cls_token = self.encoder(image, base_h, base_w, return_class_token=True)
        # features: (B, encoder_dim, base_h, base_w) — flatten spatial dims
        semantics = features.flatten(2).transpose(1, 2)  # (B, base_h*base_w, encoder_dim)
        return semantics, cls_token

    def forward_semantics_full(self, image: torch.Tensor):
        """
        Extract CLS + register + patch tokens from the frozen DINOv2 encoder.

        Returns the three token groups separately so callers can assemble them into
        any desired sequence order.

        Returns:
            patch_tokens: (B, H//14 * W//14, dim_out) — projected patch features.
            cls_token:    (B, dim_features)             — raw CLS token.
            reg_tokens:   (B, R, dim_features)          — raw register tokens (R=0 if none).
        """
        batch_size, _, img_h, img_w = image.shape
        base_h, base_w = img_h // 14, img_w // 14
        features, cls_token, reg_tokens = self.encoder(
            image, base_h, base_w,
            return_class_token=True,
            return_register_tokens=True,
        )
        # features: (B, dim_out, base_h, base_w)
        patch_tokens = features.flatten(2).transpose(1, 2)  # (B, N, dim_out)
        return patch_tokens, cls_token, reg_tokens
